  ...
  return (
    <div className="max-w-3xl space-y-6">
      <GlassCard className="p-6">
        <h4 className="font-bold text-tl-text">Link a new student</h4>
        <p className="mt-1 text-sm text-tl-text-muted">
          Enter the code your child generated from their TutorLink app.
        </p>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          <input
            ...
            placeholder="TL-XXXX-1234"
            className="flex-1 rounded-xl border border-tl-border bg-tl-surface px-4 py-2.5 text-sm outline-none focus:border-tl-primary dark:bg-tl-surface-muted"
          />
          <button
            type="button"
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-tl-primary-dark to-tl-primary-light px-5 py-2.5 text-sm font-semibold text-white"
          >
        ...
        {children.map(child => (
          <GlassCard key={child.id} className="flex items-center gap-4 p-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-tl-text-muted">{child.grade} · Linked {child.linkedAt}</p>
            </div>
