...
import { MOCK_CERTIFICATES } from '../../../../../../constants/studentProfileMockData';
...
export default function StudentCertificatesScreen({ navigation }: any) {
  ...
  const certificates =
    filter === 'recent' ? MOCK_CERTIFICATES.slice(0, 2) : MOCK_CERTIFICATES;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
          {(['all', 'recent'] as const).map(key => (
            ...
            >
              <Text style={{ color: filter === key ? '#fff' : '#64748B', fontWeight: '600', fontSize: 13 }}>
                {key === 'all' ? 'All' : 'Recent'}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        {certificates.length === 0 ? (
          ...
          certificates.map(cert => (
            <ProfileSectionCard key={cert.id}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                ...
                <MaterialCommunityIcons name="download-outline" size={22} color={colors.PRIMARY_COLOR} />
              </View>
            </ProfileSectionCard>
          ))
        )}
